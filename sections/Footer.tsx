...
export default function Footer({
  ...
  links = [
    ...
    { title: "Column Three", items: [ { label: "Link Eleven", href: "/" }, { label: "Link Twelve", href: "/" }, { label: "Link FourThirteenteen", href: "/" }, { label: "Link Fourteen", href: "/" }, { label: "Link Fifteen", href: "/" }, ] },
  ],
  subscribe = {
    title: "Subcribe",
  ...
  return (
    <div class="lg:container md:max-w-6xl md:mx-auto mx-5 text-sm pt-16">
          ...
          <div class="flex gap-9">
            {links?.map((link) => (
              <div>
                ...
                {link.items?.map((item) => (
                  <a class="link no-underline hover:underline block py-1" href={item.href}>{item.label}</a>
                ))}
              </div>
            ))}
          </div>
          <div class="lg:w-[40%]">
            ...
            <form class="flex flex-col gap-4">
              ...
              <div class="flex gap-4">
                <input type="text" placeholder="Enter your email" class="flex-auto input input-bordered input-primary" />
                <button type="submit" class="btn btn-outline font-normal">Subscribe</button>
              </div>
              <p class="text-xs" dangerouslySetInnerHTML={{ __html: subscribe.instructions, }}></p>
            </form>
          </div>
        </div>
          ...
          <div class="flex flex-col lg:flex-row gap-4 lg:gap-6 items-center lg:items-start">
            ...
            <div class="flex gap-2 lg:gap-6 justify-between">
              {bottomLinks?.map((item) => (
                <a class="link" href={item.href} target="_blank">{item.label}</a>
              ))}
            </div>
          </div>
